...
export class UserService {
  ...
  updateUserStatus(id: string, status: 'active' | 'inactive'): Observable<User> {
    return this.http.patch<User>(`${this.API_URL}/users/${id}/status`, { status }).pipe(
      tap(updatedUser => {
        this.logService.addLog({
          action: 'update',
          entity: 'user',
          entity_id: updatedUser.id,
          details: `Updated user ${updatedUser.username} status to ${status}`
        }).subscribe();
      }),
      catchError(error => {
        console.error('Error updating user status:', error);
        return throwError(() => new Error(error.error?.message || `Failed to update user status for ID ${id}.`));
      })
    );
  }

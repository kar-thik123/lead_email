...
export class LogsComponent implements OnInit {
  ...
  entityTypes = ['lead', 'user', 'auth', 'log'];
  ...
  exportLogs(format: 'csv' | 'excel'): void {
    // Ensure only CSV format is processed
    if (format !== 'csv') {
      console.warn('Only CSV export is supported in this version.');
      this.showErrorMessage('Only CSV export is currently supported.');
      return;
    }

    this.isLoading = true;
    // Pass only 'csv' format to the service
    this.logService.exportLogs('csv', this.filterOptions).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        // File name should always be .csv
        ...
        this.isLoading = false;
        this.showSuccessMessage('Logs exported as CSV successfully');
      },
      error: (error) => {

...
app.put('/api/users/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE users 
       SET username = $1, email = $2, first_name = $3, last_name = $4, role = $5, status = $6
       WHERE id = $7
       RETURNING id, username, email, first_name, last_name, role, created_at, status`,
    ...
    await pool.query(
      'INSERT INTO logs (action, entity, entity_id, details, user_id) VALUES ($1, $2, $3, $4, $5)',
      ['update', 'user', id, `Updated user ${result.rows[0].username} status to ${status}`, req.user.id]
    );
...
app.patch('/api/users/:id/status', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('Status update result:', result.rows[0]);
...
app.post('/api/logs', authenticateToken, async (req, res) => {
  try {
    ...
    // entity_id is not required for bulk actions like 'export' or 'delete_all'
    if (!action || !entity || !details) {
      return res.status(400).json({
        error: 'Missing required fields',
        details: 'action, entity, and details are required'
      });
    }
    ...
    if (actionsRequiringEntityId.includes(action) && !entity_id) {
        return res.status(400).json({
            error: 'Missing required field: entity_id',
            details: `entity_id is required for action '${action}'`
        });
    }
...
app.get('/api/leads/stats', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('Available statuses in database:', statusDebug.rows);
    ...
    const newLeads = parseInt(newResult.rows[0].count) || 0;
    console.log('New leads:', newLeads);

    // Get followed up leads count
    const followedUpResult = await pool.query(
      'SELECT COUNT(*) FROM leads WHERE LOWER(status) = \'followed_up\'',
      []
    );
    const followedUp = parseInt(followedUpResult.rows[0].count) || 0;
    console.log('Followed up leads:', followedUp);

    // Get converted leads count (including both 'converted' and 'closed' status)
    const convertedResult = await pool.query(
      'SELECT COUNT(*) FROM leads WHERE LOWER(status) IN (\'converted\', \'closed\')',
      []
    );
    const converted = parseInt(convertedResult.rows[0].count) || 0;
    console.log('Converted/Closed leads:', converted);

    // Get leads by status (combine 'closed' with 'converted', include unknown)
    const statusResult = await pool.query(
      `SELECT 
        CASE 
          WHEN LOWER(status) IN ('converted', 'closed') THEN 'converted'
    ...
    // Get leads by source (include unknown)
    const sourceResult = await pool.query(
      'SELECT COALESCE(source, \'unknown\') as source, COUNT(*) as count FROM leads GROUP BY COALESCE(source, \'unknown\')',
      []
    );
    const bySource = sourceResult.rows.reduce((acc, row) => {
      acc[row.source] = parseInt(row.count) || 0;
      return acc;
    }, {});

    // Get leads by country (include unknown)
    const countryResult = await pool.query(
      'SELECT COALESCE(country, \'unknown\') as country, COUNT(*) as count FROM leads GROUP BY COALESCE(country, \'unknown\')',
      []
    );
    const byCountry = countryResult.rows.reduce((acc, row) => {
      acc[row.country] = parseInt(row.count) || 0;
      return acc;
    }, {});

    // Get conversion rate history (last 6 months)
    const conversionHistoryResult = await pool.query(
      `WITH months AS (
      ...
      SELECT 
        ...
        COUNT(CASE WHEN LOWER(leads.status) IN ('converted', 'closed') THEN 1 END) as converted_leads
      FROM months
      LEFT JOIN leads ON date_trunc('month', leads.created_at) = months.month
      GROUP BY months.month
      ORDER BY months.month ASC`
    );
    console.log('Conversion history:', conversionHistoryResult.rows);
    const conversionHistory = conversionHistoryResult.rows.map(row => ({
      month: row.month,
      rate: row.total_leads > 0 ? (row.converted_leads / row.total_leads) * 100 : 0
    }));

    // Get weekly lead activity (last 7 days)
    const weeklyActivityResult = await pool.query(
      ...
      SELECT 
        ...
        COUNT(leads.id) as new_leads,
        COUNT(CASE WHEN LOWER(leads.status) = 'followed_up' THEN 1 END) as follow_ups
      FROM days
      LEFT JOIN leads ON date_trunc('day', leads.created_at) = days.day
      GROUP BY days.day
...
app.get('/api/leads/export', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      const formattedData = result.rows.map(lead => ({
        'Source': lead.source || '',
        'First Name': lead.first_name || '',
        'Last Name': lead.last_name || '',
        'Designation': lead.designation || '',
        'Phone No 1': lead.phone_no_1 || '',
        'Phone No 2': lead.phone_no_2 || '',
        'Email Id 1': lead.email_id_1 || '',
        'Email Id 2': lead.email_id_2 || '',
        'Email Id 3': lead.email_id_3 || '',
        ...
        'Follow Up': lead.follow_up ? new Date(lead.follow_up).toISOString().split('T')[0] : '',
...
app.get('/api/logs/export', authenticateToken, async (req, res) => {
  console.log('--- /api/logs/export called ---');
  try {
    const { format, user_id, action, entity, startDate, endDate } = req.query;
    console.log('Export params:', { format, user_id, action, entity, startDate, endDate });
    
    let query = 'SELECT l.*, u.username FROM logs l LEFT JOIN users u ON l.created_by::uuid = u.id::uuid WHERE 1=1';
    const params = [];
    
    // Build query based on filters
    if (user_id) {
      query += ' AND l.created_by = $' + (params.length + 1) + '::uuid';
      params.push(user_id);
    }
    if (action) {
    ...
    if (entity) {
      query += ' AND l.entity = $' + (params.length + 1);
      params.push(entity);
    }
    if (startDate) {
    ...
    // Convert logs to CSV or Excel format
    let content = '';
    let filename = '';
    let contentType = '';
    ...
    if (format === 'csv') {
      // CSV headers
      content = 'Timestamp,User,Action,Entity,Entity ID,IP Address,Details\n';
      
      if (logs.length === 0) {
        // No logs, just send the header row
        filename = 'logs.csv';
        contentType = 'text/csv';
        res.setHeader('Content-Type', contentType);
        res.setHeader('Content-Disposition', `attachment; filename=${filename}`);
        console.log('No logs found, sending only header row.');
        return res.send(content);
      }
      try {
        // Add rows
        logs.forEach((log, idx) => {
          if (idx < 3) console.log('Log row', idx, log); // Log first 3 rows
          content += `${new Date(log.created_at).toISOString()},${log.username || '-'},${log.action || '-'},${log.entity || '-'},${log.entity_id || '-'},${log.ip_address || '-'},"${log.details || '-'}"\n`;
        });
      } catch (err) {
        console.error('Error generating CSV rows for logs export:', err, logs);
        return res.status(500).json({ message: 'Failed to generate CSV for logs', error: err.message });
      }
      filename = 'logs.csv';
      contentType = 'text/csv';
      res.setHeader('Content-Type', contentType);
      res.setHeader('Content-Disposition', `attachment; filename=${filename}`);
      console.log('Sending CSV file with', logs.length, 'rows.');
      res.send(content);
    ...
    // Log the export action (moved inside format checks)
    await pool.query(
      'INSERT INTO logs (action, entity, entity_id, details, created_by) VALUES ($1, $2, $3, $4, $5)',
      ['export', 'log', null, `Exported logs as ${format}`, req.user.id]
    );
